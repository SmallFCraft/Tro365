...
class ModernApp {
    ...
    addCustomValidationRules(validator, form) {
        ...
        phoneFields.forEach(field => {
            validator.addRule(field.name, FormValidator.rules.phone, 'Số điện thoại không hợp lệ');
        });
    ...
    showFieldError(field, message) {
        ...
        if (!errorContainer) {
            errorContainer = DOM.createElement('div', { className: 'invalid-feedback' });
    ...
    async performSearch(query, input) {
        ...
        const resultsContainer = DOM.$(input.dataset.resultsContainer || '.search-results');
        ...
        const lightbox = DOM.createElement('div', {
            ...
            innerHTML: `
                <div class="lightbox-container">
                    ...
                    <img class="lightbox-image" src="${images[startIndex].dataset.full}" alt="">
    ...
    async handleAjaxNavigation(url, pushState = true) {
        try {
            ...
            if (response.html) {
                const contentContainer = DOM.$('#main-content') || DOM.$('main') || document.body;
