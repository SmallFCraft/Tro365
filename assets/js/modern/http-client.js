...
class HttpClient {
    ...
    async request(method, url, data = null, config = {}) {
        ...
        try {
            ...
            // Handle HTTP errors
            if (!response.ok) {
                throw new HttpError(response.status, response.statusText, await this.parseResponse(response));
            }

            return await this.parseResponse(response);
        } catch (error) {
            if (error.name === 'AbortError') {
                throw new HttpError(408, 'Request Timeout', { message: 'Request timed out' });
            }
            throw error;
        }
    }
